#!/usr/bin/env python3
"""Run exactly a frozen set of clean or attack run-spec IDs."""
...
def parse_wrapper_args() -> tuple[set[str], str, list[str]]:
    parser = argparse.ArgumentParser(add_help=False)
    parser.add_argument("--frozen-run-spec-ids", type=Path, required=True)
    parser.add_argument(
        "--required-condition", choices=("clean", "attack", "any"), default="any"
    )
    ...
    return ids, known.required_condition, [sys.argv[0], *remaining]


TARGET_IDS, REQUIRED_CONDITION, FORWARDED_ARGV = parse_wrapper_args()
...
class FrozenFailureRunner(Original):
    def _build_plan(self, *args, **kwargs):
        ...
        observed_conditions = {
            str(getattr(spec.condition, "value", spec.condition)) for spec in selected
        }
        if REQUIRED_CONDITION != "any" and observed_conditions != {REQUIRED_CONDITION}:
            raise RuntimeError(
                f"expected only {REQUIRED_CONDITION}, got {sorted(observed_conditions)}"
            )
        print(
            "[frozen-failure-recovery] "
            f"source_plan={len(original)} selected={len(selected)} "
            f"conditions={sorted(observed_conditions)}",
            flush=True,
        )
        return selected
