#!/usr/bin/env python3
"""Run the frozen extended evidence-volume intervention against vLLM."""
...
def main() -> None:
    ...
    if manifest["sampling"] != requested_sampling:
        raise ValueError(
            f"sampling differs from frozen manifest: "
            f"{requested_sampling} != {manifest['sampling']}"
        )
    ...
    stimulus_rows = read_jsonl(prepared / "stimuli.jsonl")
    stimuli = {str(row["stimulus_id"]): row for row in stimulus_rows}
    plan = read_jsonl(prepared / "requests.jsonl")
    if content_fingerprint(stimulus_rows) != manifest["stimuli_fingerprint"]:
    ...
    if len(plan) != int(manifest["expected_requests"]):
    ...
    def execute(row: dict[str, Any]) -> dict[str, Any]:
        ...
        atomic_json(result_path(output, str(row["request_id"])), result)
        return result
    ...
    try:
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for index, future in enumerate(as_completed(futures), start=1):
                ...
                except Exception as exc:  # noqa: BLE001 - persist each request failure
                    failures.append(
                        {
